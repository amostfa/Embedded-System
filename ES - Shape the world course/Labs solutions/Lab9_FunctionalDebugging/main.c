...
// Data will be stord on this variable when changes happen to the input
//unsigned long Data_portF[50];

int main(void){  unsigned long i,last,now;
  unsigned long volatile SW1, SW2;
  ...
  while(1){
		// Flash when SW1 or SW2 are pressed (PF4,PF0)
    ...
    if(i<50){
      ...
      Data[i] = GPIO_PORTF_DATA_R&0x13;//GPIO_PORTF_DATA_R&0x02; // record PF1
			//Data_portF [i] = GPIO_PORTF_DATA_R&0x13;
      last = now;
      i++;
    }
    Delay();
		...
		GPIO_PORTF_DATA_R&=~0x02; // IF no switched is pressed, turn off LED
		}
}
}
